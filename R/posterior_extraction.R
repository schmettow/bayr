	...
	function(model,
		...
		out <-
			...
			mutate(model = as.factor(model),
						 ...
						 type = as.factor(type),
						 fixef = as.factor(fixef),
						 re_factor = as.factor(re_factor),
						 re_entity = as.factor(re_entity))
...
print.tbl_post <-
	...
	function(tbl_post, kable = by_knitr(), ...){
		...
		if(kable){ ## prepared for knitr table output, not yet working
			...
			cat("\n** Shape: \n")
			print.data.frame(shape, row.names = F)

			cat("\n** Correlations: \n")
			print.data.frame(cor, row.names = F)
...
tbl_post.brmsfit <-
	function(model, ...){
		...
		par_out <-
			par_out %>%
			distinct_(.dots = bayr:::ParameterIDCols) %>%
